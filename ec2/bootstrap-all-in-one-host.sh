...
bash ec2/install-all-in-one.sh
...
pm2 start ec2/ecosystem.all-in-one.config.cjs
pm2 save
pm2 startup || true

echo "INFO: EC2 all-in-one services started"
echo "INFO: runtime health: curl http://127.0.0.1:3000/health"
echo "INFO: shared MCP endpoint: curl -X POST http://127.0.0.1:3000/mcp"
